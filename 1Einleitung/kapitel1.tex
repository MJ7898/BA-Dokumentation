...
\begin{enumerate}
    ...
    Öffentlichkeit stoßen. \cite{gartnerhc.2016s}
    ...
    und enthusiastische Erwartungen veröffentlicht und diskutiert. \cite{gartnerhc.2016s}
    ...
    der enttäuschten Enthusiasten und der Medienvertreter auf den Tiefpunkt. \cite{gartnerhc.2016s}
    \item \textit{Pfad der Erleuchtung:} Die neue Technologie wird in dieser Phase realistisch mit ihren Stärken und Schwächen betrachtet. \cite{gartnerhc.2016s}
...
einen Schritt näher, sodass in naher Zukunft die letzte Phase, das Erreichen des Plateaus der Produktivität erzielt werden kann. Der finale 
Schritt der endgültigen Marktreife ist ein faszinierender und wichtiger Grund für meine Motivation, mich dieser Technologie und 
der dahinterstehenden Theorie zu widmen. 
Die Einsatzgebieten von \acl{AR} sind unter anderem: 
\begin{itemize}
    ...
    \item Militär
\end{itemize} 
Neben der Affinität von \acs{AR} zur Technologie bringt diese Vorteile mit sich, wie z.B. die Modernisierung von Arbeitsprozessen Herangehensweisen an Projekte. 
Ebenso wird die Fehleranfälligkeit durch menschliche Unachtsamkeit mit einer Kombination aus vielen Informationen und 
...
durch die dargebotenen, visualisierten Angaben.
\\ 
\linebreak
...
Marktstudie zu \acl{AR} der IDG Research Services und PTC untermauert das immer weiter steigende Wachstum der Anwendungen in 
...
bis 2021 erreicht werden soll. So wird umso deutlicher, dass es sich bei \acl{AR} um ein sehr zukunftsorientiertes Marktsegment 
...
Infrastruktur, die es ermöglicht, physische und virtuelle Gegenstände miteinander zu vernetzen, auch bekannt als \ac{IoT}. \cite{iot.2020}
...
Wartungen schneller durchgeführt, Defekte besser behoben, Informationen über Vorfälle an der Maschinerie in Echtzeit angezeigt, Leerläufe oder 
Fehler am Endprodukt vermieden werden. Mit solch einer Unterstützung 
...
\acl{AR} Applikation entnehmen. 
%, um z.B. eine Wartung einer Anlage schneller durchzuführen oder über Vorfälle an der Maschinerie in Echtzeit informiert zu sein. 
...
generierten Objekte, die zuvor durch die Scan-Phase erzeugt wurden, angezeigt werden und deren Informationen zur Verfügung stellen.% Im Laufe 
...
diesem Abschnitt der Arbeit Gedanken, Überlegungen und vorläufige Konzeptionen der Arbeit aufgefasst und erläutert. Dazu gehören unter anderem 
die Bedingungen der Arbeitsumgebung (\ref{chap:Arbeitsumgebung}), in der die Applikation ihren Nutzen erweist. %, mit sich bringt. 
Erweiternd dazu folgen die beiden Phasen, Scan-Phase (\ref{chap:Scan-Phase}) und Visualisierungs-Phase 
(\ref{chap:Visualisierungs-Phase}). % konzipiert wurden.
Ebenso wird das Architekturkonzept (\ref{chap:Architekturkonzept}), welches für das System 
vorgesehen war, genauesten dargelegt. 
Im Anschluss wird auf das ebenso tragende Softwarekonzept (\ref{chap:Softwarekonzept}) und einer kurze Evaluierung für das angewendete \acs{AR}-Framework 
(\ref{chap:Auswahl des AR Frameworks}) eingegangen. %Darauffolgend eine kurze Evaluierung wieso sich 
%für das angewendete \acs{AR}-Framework (\ref{chap:Auswahl des AR Frameworks}) entschieden wurde und 
%Abschließend zu 
Kapitel (\ref{chap:Konzeption}) wird abgeschlossen mit der Konkretisierung der Intension des konzipierten Datenmodell's (\ref{chap:Datenmodell}). 
...
und ein eigenes Fazit zu ziehen.
\\ 
...
Die moderne Technologie \acl{AR} %findet in aller Munde Zuspruch und 
offenbart ein weites Spektrum an Einsatzmöglichkeiten. 
...
Grafikrechner und SketchAR und der Sektor Reisen, darunter zählt der \acs{AR} gestützte Google Übersetzer zählt. Auch die Umsetzung von \acl{AR} 
...
%reichlich 
wenige Anwendungen, die allseits bekannt sind. %, bzw. ein hohes Aufsehen erregt haben. 
Im Einsatz befinden sich hier verstärkt Smart-Glasses und 
...
auf bestimmte Positionen, bzw. darzustellende Objekte an den vorgesehenen Positionen während der Dauer der Benutzung. Wird die Anwendung geschlossen, 
müssen bei einem erneuten Aufruf die Objekte neu generiert und platziert werden, das heißt sie besitzen eine kurze Gültigkeit und Lebensdauer. %begrenzen sich auf die. 
%Damit ist die Langlebigkeit der Informationen adressiert.
%Die Objekte werden bei erneutem Aufruf neu generiert und platziert. 
Dies bedeutet, 
